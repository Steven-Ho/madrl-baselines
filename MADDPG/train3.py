...
class AgentTrainer(object):
    def __init__(self, num_agents, obs_shape_list, action_shape_list, args):
        ...
        num_inputs = sum(obs_shape_list)
        num_actions = sum(action_shape_list)

        self.critics = [QNetwork(num_inputs, num_actions, args.hidden_dim).to(device=self.device) for _ in range(num_agents)]
        self.critics_target = [QNetwork(num_inputs, num_actions, args.hidden_dim).to(device=self.device) for _ in range(num_agents)]
        self.critic_optims = [Adam(self.critics[i].parameters(), lr=args.critic_lr) for i in range(num_agents)]
    ...
    def update_parameters(self, samples, batch_size, index, updates):
        ...
        with torch.no_grad():
            ...
            next_q = self.critics_target[index](next_obs_batch, next_actions) - self.alpha * next_log_p
            td_q = reward_batch_i + self.gamma * next_q

        q = self.critics[index](obs_batch, action_batch)
        q_loss = F.mse_loss(q, td_q)

        a, log_p, _ = self.policy.sample(obs_batch_i)
        all_a[:,index] = a
        all_a = torch.reshape(all_a, (batch_size, -1))
        q_p = self.critics[index](obs_batch, all_a)
        policy_loss = (self.alpha * log_p - q_p).mean()

        self.critic_optims[index].zero_grad()
        q_loss.backward()
        self.critic_optims[index].step()
        ...
        if updates % self.target_update_interval == 0:
            soft_update(self.critics_target[index], self.critics[index], self.tau)
    ...
    def save_model(self, index, env_name, suffix="", actor_path=None, critic_path=None):
        ...
        torch.save(self.critics[index].state_dict(), critic_path)        
    ...
    def load_model(self, index, actor_path=None, critic_path=None, env_name=None, suffix=""):
        ...
        if critic_path is not None:
            self.critics[index].load_state_dict(torch.load(critic_path))
        elif env_name is not None:
            critic_path = "models/maddpg_critic_{}_{}_{}".format(env_name, suffix, index)
            self.critics[index].load_state_dict(torch.load(critic_path))

        hard_update(self.critic_target[index], self.critic[index])
        hard_update(self.policy_target, self.policy)